<script lang="ts">
	...
	// AKP05E_552A boot-logo coordinates on its 800 × 480 virtual canvas.
	...
	// 800 × 130 strip; its four action zones are not separate physical screens.
	const AKP05_MASK = {
		keySize: 130,
		keyX: [0, 170, 340, 510, 680],
		keyY: [0, 170],
		touchStrip: { x: 0, y: 340, width: 800, height: 130 },
	};
...
<div class="relative min-h-0 min-w-0 max-w-full">
	...
	<section class="flex min-h-0 min-w-0 items-center justify-center pt-2 pb-12">
			...
			{#if showAkp05Mask}
				<svg class="pointer-events-none absolute inset-0 h-full w-full" viewBox="0 0 800 480" preserveAspectRatio="none" aria-hidden="true">
					...
					<g fill="none" stroke="rgba(255, 255, 255, 0.38)" stroke-width="2">
						{#each AKP05_MASK.keyY as y}
							{#each AKP05_MASK.keyX as x}
								<rect {x} {y} width={AKP05_MASK.keySize} height={AKP05_MASK.keySize} rx="11" />
							{/each}
						{/each}
